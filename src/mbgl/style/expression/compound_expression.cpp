...
private:
    ...
    EvaluationResult applyImpl(const EvaluationContext& evaluationParameters, const Args& args, std::index_sequence<I...>) const {
        ...
        for (const auto& arg : args) {
            ...
            evaluated.push_back(std::move(*evaluatedArg));
        }
        const R value = evaluate(*fromExpressionValue<std::decay_t<Params>>(evaluated.at(I))...);
...
private:
    ...
    EvaluationResult applyImpl(const EvaluationContext& evaluationParameters, const Args& args, std::index_sequence<I...>) const {
        ...
        const R value = evaluate(evaluationParameters, *fromExpressionValue<std::decay_t<Params>>(evaluated.at(I))...);
        if (!value) return value.error();
        return *value;
    }
